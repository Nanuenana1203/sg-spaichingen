...
type DienstSlot = {
  ...
  anzahl_gebucht?: number; // nur im öffentlichen Modus
  dienst_zeilen: Zeile[];
};
type Dienst = {
  id: number;
...
function isFlexible(slot: DienstSlot): boolean {
  ...
  if (slot.dauer_minuten) {
    ...
    const windowMin = h2 * 60 + m2 - (h1 * 60 + m1);
    if (windowMin !== slot.dauer_minuten) return true;
  }
  return false;
}
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-5xl px-6 py-8">
        ...
        {/* Schritt 1: Name + Telefon (nur für Buchung nötig) */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
            ...
            <p className="text-xs text-slate-500">
              Schritt 2 → Dienst auswählen und auf <strong>Buchen</strong> klicken.
            </p>
          </div>
        </div>
                ...
                <div key={dienst.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  <div className="divide-y divide-slate-100">
                    {slots.map(slot => {
                      ...
                      const gebuchteZeilen = zeilen.filter(z => z.name);
                      const freieZeilen = zeilen.filter(z => !z.name);
                            ...
                            <div className={`px-5 py-4 flex items-center justify-between gap-4 ${isOpen ? "" : "hover:bg-slate-50"}`}>
                              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
                                <span className="font-medium text-slate-800">
                                  {tage.length > 1
                                    ? `${fmtDate(slot.datum_von)} – ${fmtDate(slot.datum_bis)}`
                                    : fmtDate(slot.datum_von)}
                                </span>
                                <span className="text-slate-500">
                                  {fmtTime(slot.uhrzeit_von)} – {fmtTime(slot.uhrzeit_bis)}
                                </span>
                                {slot.dauer_minuten && (
                                  <span className="text-slate-400 text-xs">Dauer: {fmtDauer(slot.dauer_minuten)}</span>
                                )}
                                {flex && (
                              ...
                              <div className="flex items-center gap-3 shrink-0">
                                ...
                                {!ausgebucht && (
                                  <button
                                    ...
                                    className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${isOpen
                            ...
                            {isLoggedIn && (gebuchteZeilen.length > 0 || freieZeilen.length > 0) && (
                              <div className="mx-5 mb-4 rounded-xl border border-slate-200 overflow-hidden">
                                <table className="w-full">
                                  ...
                                  <tbody>
                                    {zeilen.sort((a, b) => a.nummer - b.nummer).map(z => (
                                      <tr key={z.id} className={`border-t border-slate-100 ${z.name ? "" : "bg-slate-50/50"}`}>
                                        ...
                                        <td className="px-3 py-2 text-sm text-slate-600">
                                          {z.buchung_von ? `${fmtTime(z.buchung_von)} – ${fmtTime(z.buchung_bis)}` : (z.name ? "–" : "")}
                                        </td>
