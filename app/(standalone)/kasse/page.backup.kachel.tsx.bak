...
export default function KassePage() {
  ...
  const kachelArtikel = useMemo(
    () => artikel.filter(a => a.kachel === true || a.kachel === 1),
    [artikel]
  );
  ...
  function preis(a: Artikel, gruppe?: number | null) {
    ...
    const p = a[key] as number | undefined;
    return p ?? a.preis1 ?? 0;
  }
                  ...
                  <tbody>
                    ...
                    {filteredArtikel.map(a => (
                      <tr key={a.id} className="border-t hover:bg-blue-50">
                        ...
                        <td className="px-4 py-2 text-right">
                          <button onClick={() => addArtikel(a)} className="px-3 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700">
                            Übernehmen
                          </button>
                        </td>
                      </tr>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              {kachelArtikel.map(a => (
                <button
                  ...
                  className="rounded-lg border px-3 py-3 text-left hover:shadow-sm hover:-translate-y-0.5 transition text-sm bg-white"
