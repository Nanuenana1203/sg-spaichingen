...
export default function EditMitgliedVerwaltung() {
  ...
  async function save(e: React.FormEvent) {
    ...
    if (r.ok) router.push("/mitgliederverwaltung");
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-4xl px-6 py-8">
        <h1 className="text-2xl font-bold text-slate-900 mb-6">Mitglied bearbeiten</h1>
        ...
        <form onSubmit={save} className="bg-white rounded-2xl border border-slate-200 shadow-sm">
          <div className="p-6 grid grid-cols-12 gap-4">
            ...
            {/* Hier kommen später zusätzliche Felder für die Mitgliederverwaltung */}
          ...
          <div className="flex gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
            ...
            <button type="button" onClick={() => router.push("/mitgliederverwaltung")} className="px-4 py-2 rounded-lg bg-slate-100 text-slate-700 text-sm font-medium hover:bg-slate-200 transition-colors">
              Abbrechen
            </button>
          </div>
        </form>
