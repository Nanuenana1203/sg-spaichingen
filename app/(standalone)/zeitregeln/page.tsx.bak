...
export default function ZeitregelnPage() {
  ...
  function fmtTime(s?:string|null) {
    if (!s) return "—";
    // Supabase liefert "HH:MM:SS" -> nur HH:MM zeigen
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await fetch("/api/zeitregeln", { cache: "no-store" });
      const data = await r.json().catch(()=>[]);
      const arr = Array.isArray(data) ? data as Regel[] : [];
      // Fallbacks für weekday_name
      setRows(arr.map(x => ({
        ...x,
  ...
  async function delRegel(id:number) {
    ...
    setLoading(true);
    try {
      const r = await fetch(`/api/zeitregeln?id=${encodeURIComponent(String(id))}`, { method:"DELETE" });
