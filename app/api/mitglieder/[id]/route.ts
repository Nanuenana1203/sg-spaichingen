...
export async function PUT(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (body?.preisgruppe !== undefined) row.preisgruppe = Number.isFinite(+String(body.preisgruppe)) ? parseInt(String(body.preisgruppe), 10) : null;
  if (body?.ausweisnr   !== undefined) row.ausweisnr   = body.ausweisnr || null;
  if (body?.geburtsdatum !== undefined) row.geburtsdatum = body.geburtsdatum || null;
  ...
  let rows: unknown[] = []; try { rows = JSON.parse(t); } catch {}
  return NextResponse.json({ ok: true, user: (rows as Record<string, unknown>[])?.[0] ?? null });
}
