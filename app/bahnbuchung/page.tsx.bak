...
export default function BahnbuchungPage() {
  ...
  const calendar = useMemo(() => {
    ...
    const cells: { label: string; date?: string; available?: boolean; isSelected?: boolean; isPast?: boolean; isToday?: boolean }[] = [];
    ...
    <div className="max-w-[1200px] mx-auto p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <Link href="/bahnbuchung-storno" className="px-3 py-2 rounded border text-sm bg-red-600 hover:bg-red-700">Buchung stornieren</Link>
          <Suspense fallback={null}><Suspense fallback={null}><BackLink className="px-3 py-2 rounded border text-sm">Zurück</BackLink></Suspense></Suspense>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 items-end gap-4 mb-4">
        ...
        <div>
          ...
          <input type="email" className="w-full px-3 py-2 rounded border" value={email} onChange={e => setEmail(e.target.value)} />
        </div>
        <div className="text-sm text-gray-600">
          1) Name & E-Mail eingeben · 2) Datum wählen · 3) Slot klicken
        </div>
      </div>
      ...
      <div className="flex gap-6">
        ...
        <div className="rounded-2xl border shadow p-3 w-[345px]">
          {/* Kopf mit kleinen Pfeilen */}
          <div className="flex items-center gap-3 mb-2">
            <button
              aria-label="Vorheriger Monat"
              onClick={() => setMonthDate(addMonths(monthDate, -1))}
              className="w-7 h-7 rounded border text-xs inline-flex items-center justify-center">◀</button>
          ...
          {selectedDate && (
            <div className="mt-2 text-xs">Ausgewähltes Datum: <b>{selectedDate}</b></div>
          )}
        </div>
                ...
                <div key={b.bahn_id} className="rounded-2xl border shadow p-3 bg-gray-50">
                  ...
                  <div className="flex flex-col gap-2">
                    {b.slots.map((s, idx) => {
                      ...
                      const title = booked
                        ? `Gebucht${s.by ? " von " + s.by : ""}`
                        : `Buchen ${s.start}–${s.end}`;
                      ...
                      return (
                        <button
                          ...
                          title={title}
                        >
                          {s.start}–{s.end}{booked ? (s.by ? ` · ${s.by}` : "") : ""}
                        </button>
                      );
                    })}
                  </div>
