...
export default function DienstbuchungPublicPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await fetch("/api/dienste-public", { cache: "no-store" });
      const d = await r.json().catch(() => []);
      setDienste(Array.isArray(d) ? d.filter((di: Dienst) => di.aktiv) : []);
  ...
  async function buchen(slot: DienstSlot) {
    if (!name.trim() || !telefon.trim()) { toast.warning("Bitte zuerst Name und Telefonnummer eingeben."); return; }
    if (!active?.datum || !active?.zeitVon || !active?.zeitBis) { toast.warning("Bitte Datum und Uhrzeit auswählen."); return; }
    setBooking(true);
    toast.info("Buchung wird durchgeführt…");
    try {
      const r = await fetch("/api/dienst-buchen-public", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ slot_id: slot.id, name: name.trim(), telefon: telefon.trim(), datum: active.datum, buchung_von: active.zeitVon, buchung_bis: active.zeitBis }),
      });
      const j = await r.json().catch(() => ({}));
      if (!r.ok || j?.ok !== true) {
        toast.error(j?.error === "AUSGEBUCHT" ? "Dieser Dienst ist bereits ausgebucht." : "Buchung fehlgeschlagen.");
        return;
      }
